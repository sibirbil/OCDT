...
if dataset == 'class':
    perf_df_dataset = pd.read_csv(f'{base_folder}/data/results/perf_df_class.csv')
    ...
    perf_df = pd.read_csv('data/results/perf_df_scores_cars.csv')
...
if dataset == 'class':
    pd.pivot_table(perf_df_plot, values='mse', columns='run', 
                   index=['fold', 'num_targets', 'num_rows']).to_csv(
                       f'{base_folder}/data/results/perf_df_class_mean.csv')
else:
    pd.pivot_table(perf_df_plot, values='mse', columns='run', 
                   index='fold').to_csv(f'{base_folder}/data/results/perf_df_{dataset}_mean.csv')
